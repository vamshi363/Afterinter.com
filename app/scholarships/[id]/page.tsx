...
export default function ScholarshipDetailsPage() {
  ...
  if (!scholar) return <div className="p-20 text-center text-xl font-bold">Scholarship not found</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <motion.div 
        ...
        className="bg-white dark:bg-slate-900 rounded-[3rem] p-8 md:p-10 border border-slate-200 dark:border-slate-800 shadow-xl"
      >
        <div className="flex flex-col md:flex-row gap-8 items-start mb-10 pb-10 border-b dark:border-slate-800">
          ...
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-black mb-4 leading-tight text-slate-900 dark:text-white">{scholar.name}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
          <div className="space-y-8">
            <section>
              <h3 className="text-lg font-bold mb-4 flex items-center text-slate-900 dark:text-white"><CheckCircle2 size={20} className="mr-2 text-primary-teal" /> Eligibility Criteria</h3>
            ...
            <section>
              <h3 className="text-lg font-bold mb-4 flex items-center text-slate-900 dark:text-white"><FileText size={20} className="mr-2 text-primary-teal" /> Required Documents</h3>
              <ul className="grid grid-cols-1 gap-3">
                {scholar.documents.map((doc, idx) => (
                  <li key={idx} className="flex items-center space-x-3 text-sm p-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl hover:border-primary-teal/50 transition-colors text-slate-700 dark:text-slate-300">
          ...
          <div className="space-y-8">
            <section className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-lg">
            ...
            <div className="p-6 bg-orange-50 dark:bg-orange-900/10 border border-orange-200 dark:border-orange-800 rounded-[2rem] flex items-start space-x-4">
              ...
              <div className="text-sm">
                ...
                <p className="text-orange-700/80 dark:text-orange-500/80 leading-relaxed">Never pay any processing fees to agents. Scholarships on this portal are applied for through government or official trust websites for free.</p>
              </div>
            </div>
          </div>
        </div>
